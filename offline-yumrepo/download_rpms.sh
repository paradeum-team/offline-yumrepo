...
kube(){
	rm -rf kube
	yum --downloadonly --downloaddir=./kube install -y kubelet-1.20.4
	yum --downloadonly --downloaddir=./kube install -y kubeadm-1.20.4
	yum --downloadonly --downloaddir=./kube install -y kubectl-1.20.4
}
bash_completion(){
...
replease_rpms(){
	...
	rm -rf base docker ansible update kube bash_completion
